...
var CitySelect = function (_Component) {
    ...
    CitySelect.prototype.componentWillReceiveProps = function componentWillReceiveProps(nextProps) {
        ...
        var provinceData = this.state.provinceData;
        var index = this.getIndex('province', province);

        var cityIndexTemp = 0;
        for (var i = 0; i < provinceData[index].city.length; i++) {
            if(city == provinceData[index].city[i].name) {
                cityIndexTemp = i;
                break;
            }
        }
        ...
        this.setState({
            ...
            provinceIndex: index,
            cityIndex: cityIndexTemp,
            areas: provinceData[index].city[cityIndexTemp].area,
        });
        // this.handleProvinceChange(province);

        this.onChange(province, city, area);
    };
