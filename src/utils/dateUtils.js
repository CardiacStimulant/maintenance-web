/**
 * 仅用于转换格式为"yyyy-MM-dd HH:mm:ss"的String类型日期
 * formatStr例"yyyy-MM-dd HH:mm:ss"
 * @param {String} stringDate 
 * @param {String} formatStr 
 */
export const StringDateFormat  = (stringDate, formatStr) => {
    let formatDate = new Date(Date.parse(stringDate.replace(/-/g,"/"))).format(formatStr);
    return formatDate;
}

/**
 * 毫秒时间戳格式化为formatStr(例"yyyy-MM-dd HH:mm:ss")
 * @param {Number} millisecond 
 * @param {String} formatStr 
 */
export const millisecondFormat = (millisecond, formatStr) => {
    let formatDate = new Date(millisecond).format(formatStr);
    return formatDate;
}
